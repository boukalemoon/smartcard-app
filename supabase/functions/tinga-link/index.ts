// Tınga masaüstü uygulaması bağlantı doğrulama endpoint'i.
//
// Tınga, kullanıcı QRtım'de giriş yaptıktan sonra aldığı tek kullanımlık token'ı
// buraya gönderir. Token doğrulanır ve kullanıcının QRtım kimliği döndürülür.
//
// Güvenlik: Tınga'ya yalnızca herkese açık kimlik alanları (ad, kullanıcı adı,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (tok.partner !== "tinga") return json({ valid: false, error: "Geçersiz token" }, 401);
